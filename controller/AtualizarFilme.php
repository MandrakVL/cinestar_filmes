...
class CadastrarFilme
{
    ...
    public function __construct() {
        ...
        if (isset($_POST['descricao_filme'])) {
            $this->descricao = mysqli_real_escape_string($this->mysqlConexao, $_POST['descricao_filme']);
        }
    ...
    public function adicionarFilme()
    {
        ...
        $query = "UPDATE filmes SET nome='$this->nome', descricao='$this->descricao', imagem='$this->imagem', ano_lancamento='$this->ano_lancamento', trailer='$this->trailer' WHERE id=$this->id";
        $queryExecute = mysqli_query($mysqlConexao, $query);

        header("location: http://localhost/logintst/views/components/painel.php?pagina=editfilme&atualizado=$this->nome");
    }
